...
namespace Connect4
{
    ...
    public class Game
    {
        ...
        public Game(int col, int row)
        {
            bordcol = col;
            bordrow = row;
            full = new List<int>(); // eh lzmet de ?
            for (int i = 0; i < bordcol; i++)
            {
                full.Add(bordrow - 1);
            }
            board = new state[bordcol, bordrow]; //to build rows and columns
            BoardWidth = bordcol * 100; //width
            BoardHeight = bordrow * 100; //height
            player1 = true;
            player2 = false;
            pieceColor = Color.Red;//
            for (int i = 0; i < bordcol; i++)
            {
                for (int j = 0; j < bordrow; j++)
                {
        ...
        public Game(int x, int y, Color pcolor, int col, int row)
        {
            bordcol = col;
            bordrow = row;
            board = new state[bordcol, bordrow];
            full = new List<int>();
            for (int i = 0; i < bordcol; i++)
            {
                full.Add(bordrow - 1);
        ...
        public void Reset()
        {
            full = new List<int>(7) { 5, 5, 5, 5, 5, 5, 5 }; //for reset <---
            player1 = true;
            player2 = false;
            pieceColor = Color.Red;
            for (int i = 0; i < bordcol; i++)
        ...
        public void drawBoard(PaintEventArgs e)
        {
            ...
            int lineXi = 0, lineXf = bordcol * 100;
            int lineYi = 0, lineYf = bordrow * 100;
        ...
        public void drawGamePiece(MouseEventArgs e, Graphics f)
        {
            ...
            if (full[xlocal] >= 0)
            {
                System.Console.WriteLine(full[xlocal]);
        ...
        public Color WinningPlayer()
        {
            ...
            int colIndex = bordcol - 1;
            int rowIndex = bordrow - 1;
